...
function cleanBlueApp() {
  return {
    ...
    summaryCards: [
      ...
      { label: "Total Kelompok", value: 0, icon: "👥", color: "bg-emerald-500" },
    ...
    // ── Data ──────────────────────────────────────────────────
    sheetURL:
      "https://docs.google.com/spreadsheets/d/1WvFGRiuiJiGGdhLp1EqtKPhqHi7yNn8UipH_VRKzFo/gviz/tq?tqx=out:json",
    rawData: [],
    filtered: [],
    searchQuery: "",
    totalCount: 0,
    ...
    async init() {
      ...
      try {
        ...
        // ✅ Render chart dengan delay bertahap untuk mobile
        const renderCharts = () => {
          this.updatePieChart();
          this.updateKecamatanChart();
        };
        setTimeout(renderCharts, 200);
        setTimeout(renderCharts, 600); // retry untuk mobile
      } catch (err) {
        ...
        m.bindPopup(`
          <div style="min-width:200px;font-family:sans-serif;">
            ...
            <div style="font-size:11px;color:#374151;line-height:1.6;">
              <b>Jenis Usaha:</b> <span style="color:${color};font-weight:600;">${r.jenis_usaha || "-"}</span><br>
              <b>Kelompok:</b> ${r.kelompok || "-"}<br>
    ...
    updateSummaryCards() {
      ...
      this.summaryCards = [
        ...
        { label: "Total Kelompok", value: kelompokSet.size.toLocaleString(), icon: "👥", color: "bg-emerald-500" },
    ...
    // ══════════════════════════════════════════════════════════════
    // STATISTIK & CHART (✅ PERBAIKAN CHART MOBILE)
    // ══════════════════════════════════════════════════════════════
    ...
    // ✅ PERBAIKAN: Update Pie Chart dengan requestAnimationFrame
    updatePieChart() {
      ...
      const makeChart = (canvasId) => {
        // ✅ Tunggu requestAnimationFrame agar DOM stabil
        requestAnimationFrame(() => {
          ...
          // ✅ Paksa resize setelah chart dibuat
          setTimeout(() => { try { chart.resize(); } catch(_) {} }, 100);

          if (canvasId === "barChart") this.barChart = chart;
          if (canvasId === "barChartMobile") this.barChartMobile = chart;
        });
      };

      makeChart("barChart");
      makeChart("barChartMobile");
    },
    ...
    // ✅ PERBAIKAN: Update Kecamatan Chart dengan requestAnimationFrame
    updateKecamatanChart() {
      ...
      requestAnimationFrame(() => {
        ...
        setTimeout(() => { try { this._kecChart.resize(); } catch(_) {} }, 100);
      });
    },
